...
export async function generateStaticParams() {
  try {
    ...
    if (brands.length === 0) {
      console.warn('⚠️ [brands/[slug]/solutions] No brands found, using fallback brand list');
    ...
    brands
      ...
      .forEach(brand => {
        ...
        if (/^[A-Z]/.test(originalSlug)) {
          ...
          console.log(`🔧 [brands/[slug]/solutions] Creating static param (original): ${brand.name} -> ${originalEncoded}`);
          ...
          console.log(`🔧 [brands/[slug]/solutions] Creating static param (lowercase): ${brand.name} -> ${lowercaseEncoded}`);
        } else {
          // 中文品牌或其他，只生成一个版本
          const slug = encodeURIComponent(originalSlug);
          staticParams.push({ slug });
          console.log(`🔧 [brands/[slug]/solutions] Creating static param: ${brand.name} -> ${slug}`);
        }
      });

    console.log(`🔧 [brands/[slug]/solutions] Generated ${staticParams.length} static params`);
    return staticParams;
  } catch (error) {
    ...
    console.log(`🔧 [brands/[slug]/solutions] Using fallback brands: ${fallbackBrands.length} brands`);
    const fallbackParams = [];
    fallbackBrands.forEach(brandName => {
      if (/^[A-Z]/.test(brandName)) {
        // 英文品牌生成大写和小写版本
