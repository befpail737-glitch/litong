...
export async function generateStaticParams() {
  try {
    console.log('🔧 [generateStaticParams] Starting brand solutions page static generation...');

    // 使用更可靠的slug获取方式，确保与主品牌页面一致
    const brandSlugs = await getBrandSlugsOnly(50);

    if (!brandSlugs || brandSlugs.length === 0) {
      console.warn('⚠️ [generateStaticParams] No brand slugs found for solutions, using comprehensive fallback');
      ...
      console.log(`📋 [generateStaticParams] Generated ${fallbackParams.length} fallback params for solutions`);
    ...
    console.log(`✅ [generateStaticParams] Generated ${validParams.length} static params for brand solutions`);

    return validParams;
  } catch (error) {
    console.error('❌ [generateStaticParams] Error generating static params for brand solutions:', error);
    // 返回基本的fallback参数
    return [
                  ...
                  <div key={solution._id} className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                    {solution.image && (
                      <div className="aspect-video relative bg-gray-100">
                        <Image
                          src={safeImageUrl(solution.image, { width: 600, height: 300 })}
                          alt={solution.title}
                          fill
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex gap-3">
                        <Button variant="outline" size="sm" className="flex-1" asChild>
                          <Link href={generateSolutionUrl(locale, brand.slug || brand.name, solution.slug)}>
                            了解详情
                          </Link>
                        </Button>
