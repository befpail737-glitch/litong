...
export const parseExcelData = (file: File): Promise<{ products: ProductData[], columns: FilterColumn[] }> => {
    ...
    try {
      ...
      if (file.name.endsWith('.csv')) {
        ...
        worksheet.eachRow((row, rowNumber) => {
          ...
          // Skip the first element as it's undefined for some reason in exceljs
          data.push(rowData.slice(1).map(cell => cell ? String(cell).trim() : ''));
        });
  ...
  function processData(data: string[][], resolve: Function, reject: Function) {
    try {
      if (data.length < 2) {
